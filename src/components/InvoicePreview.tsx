    ...
    <div className="flex flex-col h-full overflow-hidden bg-slate-50">
      ...
      <div className="p-4 flex gap-4 shrink-0 bg-white border-b border-slate-200 z-10 sticky top-0 justify-center no-print">
        <button
          ...
          className="bg-slate-900 text-white rounded-lg px-6 py-2.5 flex items-center gap-2 hover:bg-slate-800 transition-all shadow-sm font-bold text-sm"
        >
          <Download size={18} /> Download PDF
        </button>
        <button
          onClick={onDownloadPNG}
          className="bg-white border border-slate-200 text-slate-700 rounded-lg px-6 py-2.5 flex items-center gap-2 hover:bg-slate-50 transition-all shadow-sm font-bold text-sm"
        >
          <ImageIcon size={18} /> Export Image
        </button>
        <button
          onClick={onPrint}
          className="bg-blue-600 text-white rounded-lg px-6 py-2.5 flex items-center gap-2 hover:bg-blue-700 transition-all shadow-sm font-bold text-sm"
        >
      ...
      <div className="flex-1 overflow-auto custom-scrollbar flex justify-center p-12 print:p-0 bg-slate-100/50">
        <div
          id="invoice-canvas"
          className="bg-white mx-auto shadow-2xl print:shadow-none print:!m-0 text-slate-800 font-sans relative flex flex-col p-8 border border-slate-200"
          style={{ width: '842px', minHeight: '1190px' }}
        >
          {/* Header Row */}
          <div className="flex justify-between items-start mb-6">
            <div className="flex flex-col gap-3 items-start">
              {companyInfo.logo ? (
                ...
                <div className="w-16 h-16 rounded-xl bg-slate-100 flex items-center justify-center text-3xl font-black text-slate-300">
                  V
                </div>
              )}
              <div>
                <h1 className="text-xl font-black text-slate-900 uppercase tracking-wide leading-tight">
                  {companyInfo.name || 'VELAAN MART AGRITECH PRIVATE LIMITED'}
                </h1>
                <div className="text-[11px] text-slate-600 leading-tight mt-1">
                  {companyInfo.address ||
                    'Velaan Mart, FORUM - TABIF, Navalurkottapattu, Trichy Tamil Nadu 620027, India'}
                  <div className="font-bold mt-1">
                    GSTIN {companyInfo.gstNumber || '33AAKCV5016C1ZO'}
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 border border-slate-200 text-[11px] mb-6">
            <div className="border-r border-slate-200">
              {[
                ...
                <div
                  key={idx}
                  className="grid grid-cols-2 p-1 border-b border-slate-100 last:border-0"
                >
            ...
            <div>
              {[
                [
                  'Place Of Supply',
                  `: ${companyInfo.placeOfSupply || 'Tamil Nadu (33)'}`,
                ],
          ...
          <div className="grid grid-cols-2 gap-0 border-t border-x border-slate-200 bg-slate-50 text-[11px] font-bold">
            <div className="p-1 px-3 border-r border-slate-200 uppercase tracking-widest text-slate-700">
              Bill To
            </div>
            <div className="p-1 px-3 uppercase tracking-widest text-slate-700">
              Ship To
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 border border-slate-200 text-[11px] mb-6">
            <div className="p-3 border-r border-slate-200 min-h-[80px]">
              <p className="font-black text-slate-900 mb-1">
                {customerInfo.name ||
                  'The Warden, Institute of Agriculture Hostels, Vamban'}
              </p>
              <p className="text-slate-500 whitespace-pre-wrap leading-relaxed">
                {customerInfo.address ||
                  'Vamban, PUDUKKOTTAI, 622303 Tamil Nadu, India'}
              </p>
            </div>
            <div className="p-3">
              <p className="font-black text-slate-900 mb-1">
                {customerInfo.name || 'Vamban'}
              </p>
              <p className="text-slate-500 whitespace-pre-wrap leading-relaxed">
                {customerInfo.address ||
                  'Vamban, PUDUKKOTTAI, 622303 Tamil Nadu, India'}
            ...
            <table className="w-full text-[10px] border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200 font-bold text-slate-700">
                  ...
                  <th className="p-1.5 border-r border-slate-200 text-left">
                    Item & Description
                  </th>
                  ...
                  <th className="p-1.5 border-r border-slate-200 text-right w-20">
                    Rate
                  </th>
                  <th
                ...
                <tr className="bg-slate-50 border-b border-slate-200 text-[9px] text-slate-600">
                  <th colSpan={5}></th>
                  <th className="p-1 border-r border-slate-200 text-center">
                    %
                  </th>
                  <th className="p-1 border-r border-slate-200 text-center">
                    Amt
              ...
              <tbody>
                {items.map((item, index) => {
                  ...
                  const cgstAmt =
                    (item.quantity * item.unitPrice * cgstRate) / 100;
                  return (
                    <tr
                      key={item.id}
                      className="border-b border-slate-100 last:border-0 h-10"
                    >
                      <td className="p-1.5 border-r border-slate-200 text-center text-slate-500">
                        {index + 1}
                      </td>
          ...
          <div className="flex border-x border-b border-slate-200 text-[10px]">
            <div className="flex-1 p-4 border-r border-slate-200 space-y-4">
              ...
              <div>
                ...
                <p className="text-[11px] font-black italic text-slate-900 leading-tight whitespace-normal">
              ...
              {terms && (
                <div>
                  <p className="font-bold text-slate-500 mb-0.5">
                    Terms & Conditions
                  </p>
                  <p className="text-slate-500 text-[9px] leading-tight whitespace-pre-wrap">
                    {terms}
                  </p>
                </div>
              )}
            ...
            <div className="w-64 shrink-0 bg-slate-50/20">
              {[
                ...
                [
                  `CGST (${(taxAmount || 0) / 2 > 0 ? ((taxAmount / subtotal) * 50).toFixed(1) + '%' : '0%'})`,
                  (taxAmount / 2).toLocaleString('en-IN', {
                    minimumFractionDigits: 2,
                  }),
                ],
                [
                  `SGST (${(taxAmount || 0) / 2 > 0 ? ((taxAmount / subtotal) * 50).toFixed(1) + '%' : '0%'})`,
                  (taxAmount / 2).toLocaleString('en-IN', {
                    minimumFractionDigits: 2,
                  }),
                ],
                ...
                <div
                  key={idx}
                  className="p-2 px-3 border-b border-slate-100 flex justify-between"
                >
              ...
              <div className="mt-12 text-center p-4">
                <p className="text-[10px] uppercase font-bold text-slate-400 tracking-widest mb-16">
          ...
          <div className="mt-auto text-right text-[9px] text-slate-300">1</div>
        </div>
      </div>
    </div>
  );
